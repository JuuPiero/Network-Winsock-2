...
unsigned __stdcall echoThread(void *param){
	...
	int ret;
	SOCKET connectedSocket = (SOCKET) param;
	char *clientIP = (char*)((struct sockaddr_in *)param)->sin_addr.s_addr;
	int clientPort = ntohs(((struct sockaddr_in *)param)->sin_port);
	ret = recv(connectedSocket, buff, BUFF_SIZE, 0);
	if (ret == SOCKET_ERROR)
		printf("Error %d: Cannot receive data.\n", WSAGetLastError());
	else if (ret == 0)
...
int main(int argc, char* argv[])
{
	...
	while(1){
		...
		connectedSocket = accept(listenSock, (sockaddr *)& clientAddr, &clientAddrLen);
		if (connectedSocket  == SOCKET_ERROR)
			...
			_beginthreadex(0, 0, echoThread, (void *)connectedSocket, 0, 0); //start thread
		}		
	}
